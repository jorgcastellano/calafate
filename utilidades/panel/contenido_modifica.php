...
if($_POST["elimina_foto"]=="on"){
   ...
   mysql_query("UPDATE contenido SET foto = '' where clave = '$_POST[clave_contenido]'");
       }
...
mysql_query("UPDATE contenido SET  nombre_contenido='$nombre_final',texto='$texto_final',lugar='$lugar',fecha='$fecha',stock='$stock',precio='$precio',destacar_portada='$destaca_portada',destacar_subcategoria='$destaca_subcategoria' where clave = '$_POST[clave_contenido]'");


$sql = mysql_query("select * from contenido where clave = '$_POST[clave_contenido]'");
...
mysql_query("UPDATE contenido SET foto ='../categorias/$nombre_carpeta/$_POST[clave_subcategoria]/$_POST[clave_contenido]/$nombre_foto.$extension' where clave= '$_POST[clave_contenido]'");

                                      
									   
									   
									   } //if($_FILES["foto]["size"]>0
move_uploaded_file($temporal,"../categorias/$nombre_carpeta/$_POST[clave_subcategoria]/$_POST[clave_contenido]/".$nombre_foto.".".$extension); 
...
//LLAMA A LA FUNCION ADELGAZA FOTOS
   if($_FILES["foto"]["size"]>0){
	...
	$ima = "../categorias/$nombre_carpeta/".$_POST["clave_subcategoria"].'/'.$_POST["clave_contenido"].'/'.$nombre_foto.'.jpg';
    $ima2 = "../categorias/$nombre_carpeta/".$_POST["clave_subcategoria"].'/'.$_POST["clave_contenido"].'/'.$nombre_foto.'.jpg';
	
   $imagen = $ima;
   $nombre_imagen_asociada = $ima2;
   $carpeta = $_POST["clave_contenido"];
   $ruta = "../categorias/$nombre_carpeta/$_POST[clave_subcategoria]/".$carpeta.'/';
...
echo "<script>parent.location='contenido.php?contenido_elige=$_GET[contenido_elige]'</script>";
}
