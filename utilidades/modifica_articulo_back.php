...
if($_POST["borra_articulo"]=="ok"){



if($_POST["foto_articulo"]!=""){
unlink($_POST["foto_articulo"]);
}//if($_POST["foto_articulo"]!=""){


$sql_f = mysql_query("select * from fotos where clave_articulo_f = '$_GET[clave_articulo]' ");
$cta_f = mysql_num_rows($sql_f);

for($xz=1;$xz<=$cta_f;$xz++){
$lee_f = mysql_fetch_assoc($sql_f);
unlink($lee_f["foto"]);
mysql_query("DELETE from fotos where clave='$lee_f[clave]'");
}//for($xz=1;$xz<=$cta_f;$xz++){

mysql_query("DELETE from articulo where clave='$_GET[clave_articulo]'");

echo "<br>Articulo borrado<br><br>";
echo "<br><input type='button' value='Volver' onclick=location.href='panel.php'>" ;
die();
}//if($_POST["borra_articulo"]=="ok"){
...
echo "Buscar:<br>";
echo "<input type='text' name='buscar' ><br><br>";

echo "<input type='submit' value='Buscar' >";

echo "</form><br>";

echo "<a href='modifica_articulo.php' style='color:#ff0000;text-decoration:none' >[mostrar todos]</a><br><br>";
...
//---------------------------------------IMPRIME RESULTADO DE LA BUSQUEDA

echo "<div>";


$sql = mysql_query("select * from articulo where nombre_articulo like '%$_GET[buscar]%' order by nombre_articulo asc ");
$cta_sql = mysql_num_rows($sql);
...
//---------------------------------------FIN IMPRIME RESULTADO DE LA BUSQUEDA
...
echo "<form method='post' action='$_SERVER[PHP_SELF]?clave_articulo=$_GET[clave_articulo]' >";//**/

echo "<input type='hidden' name='foto_articulo' value='$lee_sql1[foto_articulo]'>";

echo "<input type='hidden' name='borra_articulo' value='ok'>";


echo "<br><br><input type='submit' value='BORRAR'>";
